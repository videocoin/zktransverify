...
template<typename ramT>
ram_example<ramT> gen_ram_example_complex_experiment(const ram_architecture_params<ramT> &ap, const size_t boot_trace_size_bound, const size_t time_bound, const bool satisfiable)
{
    libff::enter_block("Call to gen_ram_example_complex");
    ...
    const size_t input_addr = (1ul<<(ap.w-1)) + (ap.w/8); // byte address of the first input word
#else
    const size_t instr_addr = 268435458;
    const size_t input_addr = 268435458;
#endif
    ...
    {
#if DEBUG
        const unsigned long value = std::rand() % (1ul<<(2*ap.w));
        const unsigned long address = (1ul<<(ap.dwaddr_len()-1)) + i + 1;
        printf("Input value %zu at address %zu\n", value, address);
        result.boot_trace.set_trace_entry(boot_pos++, std::make_pair(address, value));
#else
        result.boot_trace.set_trace_entry(boot_pos++, std::make_pair((1ul<<(ap.dwaddr_len()-1)) + i + 1, std::rand() % (1ul<<(2*ap.w))));
#endif
