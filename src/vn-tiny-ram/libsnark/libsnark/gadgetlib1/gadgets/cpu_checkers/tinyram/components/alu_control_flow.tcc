...
//        FMT(this->annotation_prefix, " cjmp_result"));
    ...
    this->pb.add_r1cs_constraint(
            r1cs_constraint<FieldT>(
                    this->flag,
                    pb_packing_sum<FieldT>(pb_variable_array<FieldT>(this->argval2.bits.begin(), this->argval2.bits.end())) - this->pc.packed - 1,
                    this->result - this->pc.packed - 1),
            FMT(this->annotation_prefix, " cjmp_result"));
}
...
//                                  this->pb.val(this->pc.packed) + FieldT::one());
    ...
    this->pb.val(this->result) = ((this->pb.val(this->flag) == FieldT::one()) ?
                                  FieldT(this->pb.val(this->argval2.packed).as_ulong()) :
                                  this->pb.val(this->pc.packed) + FieldT::one());
}
