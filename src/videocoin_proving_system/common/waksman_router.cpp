    ...
    while (true) {
        ...
        if (right_index == n - 2 && n % 2 == 0) {//second to last in even, a link. NOTE: last is already done.
            ...
            if ((right_index % 2 == 0 && sw) ||
                (right_index % 2 == 1 && !sw)) { //top and swapped or bottom and unswapped.
                routeBot = true;
            }
...
bool data_equal(data_t *data1, data_t *data2) {
    return data1->addr == data2->addr && data1->timestamp == data2->timestamp && data1->type == data2->type &&
           data1->value == data2->value;
}

void printp(packet_t *packets, size_t n) {
    for (size_t i = 0; i < n; i++) {
        printf("data: %d,%d\tsrc: %ld\tdst: %ld, routed: %d\n", packets[i].data.addr, packets[i].data.timestamp,
               packets[i].src, packets[i].dst, packets[i].routed);
    }
}

void
wak_route(data_t *input, data_t *intermediate, data_t *output, switch_t *switches, size_t width, size_t num_switches) {
