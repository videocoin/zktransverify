...
void quicksort_key_value_pairs(char *buffer, int left, int right, long entry_size,
...
void quicksort_key_value_pairs(char *buffer, int number_of_entries, long entry_size,
                               int (*comparator)(const void *, const void *)) {
...
void partition_one_chunk(const char *filename_prefix, const char *folder_name, long offset, int chunk_id,
                         int number_of_entries, long entry_size) {
...
void sort_chunks(const char *filename_prefix, const char *folder_name, int number_of_chunks, long entry_size,
                 int (*comparator)(const void *, const void *)) {
...
void sort_one_chunk(const char *filename, const char *folder_name, long entry_size,
                    int (*comparator)(const void *, const void *)) {
    char full_filename[BUFLEN];
    snprintf(full_filename, BUFLEN - 1, "%s/%s", folder_name, filename);
    off_t file_size = get_file_size(full_filename);
...
void merge_sorted_chunks(const char *filename_prefix, const char *folder_name, int number_of_chunks, long entry_size,
                         int (*comparator)(const void *, const void *)) {
    ...
    while (true) {
        ...
        if (chunks[smallest_kv_pair_index].current_pointer >= chunks[smallest_kv_pair_index].buffer_tail) {
            ...
            chunks[smallest_kv_pair_index].buffer_tail =
                    chunks[smallest_kv_pair_index].buffer + chunks[smallest_kv_pair_index].fp.gcount();
...
void external_sort(const char *filename, const char *folder_name, int number_of_chunks, long entry_size,
                   int (*comparator)(const void *, const void *), bool partition) {
    if (partition) {
