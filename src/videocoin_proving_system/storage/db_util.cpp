...
void
generate_one_chunk_of_db_row(long offset, int chunk_id, int number_of_entries, long entry_size, const char *folder_name,
                             const char *folder_state) {
    ...
    load_array((char *) average_buf, number_of_entries * sizeof(tree_key_t), offset * sizeof(tree_key_t),
               "average_keys", folder_name);
    load_array((char *) class_buf, number_of_entries * sizeof(tree_key_t), offset * sizeof(tree_key_t), "class_keys",
               folder_name);
    load_array((char *) age_buf, number_of_entries * sizeof(tree_key_t), offset * sizeof(tree_key_t), "age_keys",
               folder_name);
    ...
    for (int k = 0; k < number_of_entries; k++) {
        ...
        key_kv_pairs_buf[k].offset = average_kv_pairs_buf[k].offset = class_kv_pairs_buf[k].offset = age_kv_pairs_buf[k].offset =
                offset + k;
    ...
    dump_array((char *) row_buf, number_of_entries * sizeof(Student_t), offset * sizeof(Student_t),
               "/block_stores/rows.db", folder_state);
...
int generate_db_rows(int number_of_rows, int number_of_chunks, int entries_per_chunk, const char *folder_state) {
    ...
    for (int i = 0; i < number_of_rows; i += entries_per_chunk) {
        ...
        printf("generating rows: finished chunks = %d progress = %0.1f%%\n", finished_chunks,
               100.0 * finished_chunks / number_of_chunks);
    }
...
void
merge_trees(const char *tree_name, const char *kv_pairs_filename, const char *folder_name, int number_of_subtrees) {
...
void build_one_subtree_bottom_up(const char *tree_name, int subtree_id, const char *kv_pairs_filename,
    ...
    if (buf_offset != subtree_size * entry_size) {
        cout << "Something is wrong with the buffer size. expected size " << subtree_size * entry_size << " actual size"
             << buf_offset << endl;
        exit(1);
...
int build_tree_bottom_up(tree_t *tree, const char *tree_name, const char *kv_pairs_filename, const char *folder_name,
                         int tree_size, int max_depth) {
    int max_number_of_subtrees = (2 << max_depth);
    ...
    for (int i = 0; i < number_of_subtrees; i++) {
        ...
        finished_chunks++;
        printf("building %s: finished subtrees = %d progress = %0.1f%%\n", tree_name, finished_chunks,
               100.0 * finished_chunks / number_of_subtrees);
    }
...
//#pragma omp section
        {
            int number_of_chunks = build_tree_bottom_up(&Average_index, "average_tree", "average_kv_pairs", FOLDER_TMP,
                                                        number_of_rows, max_depth);
            handle.Average_index = Average_index.root;
        }
//#pragma omp section
        {
            int number_of_chunks = build_tree_bottom_up(&Class_index, "class_tree", "class_kv_pairs", FOLDER_TMP,
                                                        number_of_rows, max_depth);
            handle.Class_index = Class_index.root;
        }
//#pragma omp section
        {
            int number_of_chunks = build_tree_bottom_up(&Age_index, "age_tree", "age_kv_pairs", FOLDER_TMP,
                                                        number_of_rows, max_depth);
...
void merge_into_one_db(const char *folder_state, int number_of_chunks, const char *bstore_file_name) {
    ...
    while (true) {
        ...
        if (hash_t_comparator(&previous_hash, (hash_t *) smallest_entry) > 0) {
            cout << "hey, dude, you're messed up, the key-value pairs is not sorted. "
                 << chunks[smallest_entry_index].filename << endl;
...
Student_handle_t create_db(int number_of_rows, int number_of_chunks, int max_depth, const char *bstore_file_name,
                           const char *folder_state) {
...
void merge_compressed_trees(const char *tree_name, const char *kv_pairs_filename, const char *folder_name,
                            const char *folder_state, int number_of_subtrees) {
    subtree_info_t subtrees[number_of_subtrees];
    ...
    while (number_of_subtrees > 0) {
        for (int i = 0; i < number_of_subtrees; i++) {
            ...
            dump_array((char *) &c_nodes[i], sizeof(compressed_tree_node_t),
                       subtrees[i].root_index * sizeof(compressed_tree_node_t), "/block_stores/index.db", folder_state);
            dump_array((char *) &subtrees[i].root, sizeof(hash_t), subtrees[i].root_index * sizeof(hash_t),
                       "/block_stores/hash.db", folder_state);
        }
        number_of_subtrees /= 2;
    }
    input_fp.close();
    ...
    load_array((char *) &root, sizeof(compressed_tree_node_t), subtrees[0].root_index * sizeof(compressed_tree_node_t),
               "/block_stores/index.db", folder_state);
...
hash_t build_compressed_subtree(compressed_tree_node_t *c_node_buf, hash_t *hash_buf, long &offset, long &index_offset,
                                key_value_pair_t *kv_pairs, int left, int right, int *height) {
    tree_node_t node;
...
void build_one_compressed_subtree_bottom_up(const char *tree_name, int subtree_id, long existing_index_entries,
                                            const char *kv_pairs_filename, const char *folder_name,
                                            const char *folder_state) {
    subtree_info_t subtree_info;
    char full_filename[BUFLEN];
    snprintf(full_filename, BUFLEN - 1, "%s_%d_info", tree_name, subtree_id);
    ...
    // also want to seek and read the same file. Interleaving between seek
    // and read would result in reading wrong data
    load_array((char *) kv_pairs, subtree_size * sizeof(key_value_pair_t), subtree_info.left * sizeof(key_value_pair_t),
               kv_pairs_filename, folder_name);
    subtree_info.root = build_compressed_subtree(c_node_buf, hash_buf, offset, index_offset, kv_pairs, 0,
                                                 subtree_size - 1, &(subtree_info.height));
    subtree_info.root_index = index_offset - 1;

    snprintf(full_filename, BUFLEN - 1, "%s_%d_info", tree_name, subtree_id);
    dump_array((char *) &subtree_info, sizeof(subtree_info_t), full_filename, folder_name);

    dump_array((char *) c_node_buf, subtree_size * sizeof(compressed_tree_node_t),
               (subtree_info.left + existing_index_entries) * sizeof(compressed_tree_node_t), "/block_stores/index.db",
               folder_state);
    dump_array((char *) hash_buf, subtree_size * sizeof(hash_t),
               (subtree_info.left + existing_index_entries) * sizeof(hash_t), "/block_stores/hash.db", folder_state);
...
int build_compressed_tree_bottom_up(tree_t *tree, const char *tree_name, const char *kv_pairs_filename,
                                    const char *folder_name, const char *folder_state, long existing_index_entries,
                                    int tree_size, int max_depth) {
    int max_number_of_subtrees = (2 << max_depth);
    ...
    for (int i = 0; i < number_of_subtrees; i++) {
        build_one_compressed_subtree_bottom_up(tree_name, i, existing_index_entries, kv_pairs_filename, folder_name,
                                               folder_state);
        finished_chunks++;
        printf("building %s: finished subtrees = %d progress = %0.1f%%\n", tree_name, finished_chunks,
               100.0 * finished_chunks / number_of_subtrees);
...
Student_handle_t build_compressed_index_trees(int number_of_rows, int max_depth, const char *folder_state,
                                              const char *bstore_file_name) {
...
#else
    build_compressed_tree_bottom_up(&Average_index, "average_tree", "average_kv_pairs", FOLDER_TMP, folder_state,
                                    index_file_offset, number_of_rows, max_depth);
    index_file_offset += number_of_rows;
    build_compressed_tree_bottom_up(&Class_index, "class_tree", "class_kv_pairs", FOLDER_TMP, folder_state,
                                    index_file_offset, number_of_rows, max_depth);
    index_file_offset += number_of_rows;
    build_compressed_tree_bottom_up(&Age_index, "age_tree", "age_kv_pairs", FOLDER_TMP, folder_state, index_file_offset,
                                    number_of_rows, max_depth);
...
void test_compressed_trees(Student_handle_t handle, int number_of_rows, const char *folder_state,
                           const char *bstore_file_name) {
    cout << "testing tree" << endl;
...
Student_handle_t
create_compressed_db(int number_of_rows, int number_of_chunks, int max_depth, const char *bstore_file_name,
                     const char *folder_state) {
