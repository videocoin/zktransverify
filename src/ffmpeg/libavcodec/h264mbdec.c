...
static av_cold int h4mb_decode_end(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    int i;
...
static av_cold int h4mb_decode_init(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
...
static void ff_h264_flush_change(H264Context *h)
{
    int i, j;
...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    for (i = 0; i < h->pkt.nb_nals; i++) {
        ...
        case H264_NAL_SLICE:
            ...
            if ((err = ff_h264mb_queue_decode_slice(h, nal))) {
            ...
            if (h->nb_slice_ctx_queued == max_slice_ctx) {
                if (h->avctx->hwaccel) {
                    ...
                    ret = ff_h264mb_execute_decode_slices(h);
...
static const AVClass h264mb_class = {
    .class_name = "H264MB Decoder",
...
AVCodec ff_h4mb_decoder = {
    ...
    .init                  = h4mb_decode_init,
    .close                 = h4mb_decode_end,
    .decode                = h4mb_decode_frame,
    ...
    .update_thread_context = ONLY_IF_THREADS_ENABLED(ff_h264mb_update_thread_context),
