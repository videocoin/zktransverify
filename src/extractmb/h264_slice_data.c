...
static void read_residual_block(h264_stream_t* h, bs_t* b, int* coeffLevel, int maxNumCoeff)
{
    if( !h->pps->entropy_coding_mode_flag )
    {
    	read_residual_block_cavlc( b, coeffLevel, maxNumCoeff );
    }
    else
    {
    	read_residual_block_cabac( b, coeffLevel, maxNumCoeff );
    }
}
...
//#define read_residual_block read_residual_block_cavlc
    ...
    if( MbPartPredMode( mb->mb_type, 0 ) == Intra_16x16 )
    {
        read_residual_block(h, b, mb->Intra16x16DCLevel, 16 );
    }
    for( int i8x8 = 0; i8x8 < 4; i8x8++ ) // each luma 8x8 block
    {
        if( !mb->transform_size_8x8_flag || !h->pps->entropy_coding_mode_flag )
        {
            for( int i4x4 = 0; i4x4 < 4; i4x4++ ) // each 4x4 sub-block of block
            {
                if( CodedBlockPatternLuma & ( 1 << i8x8 ) )
                {
                    if( MbPartPredMode( mb->mb_type, 0 ) == Intra_16x16 )
                    {
                        read_residual_block(h, b, mb->Intra16x16ACLevel[ i8x8 * 4 + i4x4 ], 15 );
                    }
                    else
                    {
                        read_residual_block(h, b, mb->LumaLevel[ i8x8 * 4 + i4x4 ], 16 );
                    }
                }
        ...
        {
            read_residual_block(h, b, mb->LumaLevel8x8[ i8x8 ], 64 );
        }
        else
        {
            for( int i = 0; i < 64; i++ )
    ...
    if( h->sps->chroma_format_idc != 0 )
    {
        ...
        for( int iCbCr = 0; iCbCr < 2; iCbCr++ )
        {
            ...
            {
                read_residual_block(h, b, mb->ChromaDCLevel[ iCbCr ], 4 * NumC8x8 );
            }
            else
            {
        ...
        for( int iCbCr = 0; iCbCr < 2; iCbCr++ )
        {
            for( int i8x8 = 0; i8x8 < NumC8x8; i8x8++ )
            {
                for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                {
                    ...
                    {
                        read_residual_block(h, b, mb->ChromaACLevel[ iCbCr ][ i8x8*4+i4x4 ], 15);
                    }
                    else
                    {
                        for( int i = 0; i < 15; i++ )
