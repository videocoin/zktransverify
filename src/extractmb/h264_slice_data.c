...
int IsCabac(h264_stream_t* h)
{
	return (h->pps->entropy_coding_mode_flag);
}
...
void read_debug_mb_pred(h264_stream_t* h, bs_t* b, int mb_type) {
	...
	if (MbPartPredMode(mb->mb_type, 0) == Intra_4x4
		...
		if (MbPartPredMode(mb->mb_type, 0) == Intra_4x4) {
			for (int luma4x4BlkIdx = 0; luma4x4BlkIdx < 16; luma4x4BlkIdx++) {
				...
				if (IsCabac(h)) {
					...
					mb->prev_intra4x4_pred_mode_flag[luma4x4BlkIdx] = bs_read_u(
							b, 1);
				}
				printf(
						"mb->prev_intra4x4_pred_mode_flag[ luma4x4BlkIdx ]: %d \n",
						mb->prev_intra4x4_pred_mode_flag[luma4x4BlkIdx]);
				if (!mb->prev_intra4x4_pred_mode_flag[luma4x4BlkIdx]) {
					printf("%d.%d: ", b->p - b->start, b->bits_left);
					if (IsCabac(h)) {
	                // Ram
						mb->rem_intra4x4_pred_mode[luma4x4BlkIdx] = bs_read_ae(
								b);
					} else {
						mb->rem_intra4x4_pred_mode[luma4x4BlkIdx] = bs_read_u(b,
								3);
					}
		...
		if (MbPartPredMode(mb->mb_type, 0) == Intra_8x8) {
			for (int luma8x8BlkIdx = 0; luma8x8BlkIdx < 4; luma8x8BlkIdx++) {
                ...
                // Ram
					...
					mb->prev_intra8x8_pred_mode_flag[luma8x8BlkIdx] = bs_read_u(
							b, 1);
				}
				printf(
						"mb->prev_intra8x8_pred_mode_flag[ luma8x8BlkIdx ]: %d \n",
						mb->prev_intra8x8_pred_mode_flag[luma8x8BlkIdx]);
				if (!mb->prev_intra8x8_pred_mode_flag[luma8x8BlkIdx]) {
					printf("%d.%d: ", b->p - b->start, b->bits_left);
					if (IsCabac(h)) {
	                // Ram
						mb->rem_intra8x8_pred_mode[luma8x8BlkIdx] = bs_read_ae(
								b);
					} else {
						mb->rem_intra8x8_pred_mode[luma8x8BlkIdx] = bs_read_u(b,
								3);
					}
		...
		for (int mbPartIdx = 0; mbPartIdx < NumMbPart(mb->mb_type);
				mbPartIdx++) {
...
void read_debug_sub_mb_pred(h264_stream_t* h, bs_t* b, int mb_type) {
	...
	for (int mbPartIdx = 0; mbPartIdx < 4; mbPartIdx++) {
		if ((h->pps->num_ref_idx_l0_active_minus1 > 0
				|| mb->mb_field_decoding_flag) && mb->mb_type != P_8x8ref0
		...
		if (mb->sub_mb_type[mbPartIdx] != B_Direct_8x8
			...
			for (int subMbPartIdx = 0;
				...
				for (int compIdx = 0; compIdx < 2; compIdx++) {
					...
					printf(
							"mb->mvd_l0[ mbPartIdx ][ subMbPartIdx ][ compIdx ]: %d \n",
							mb->mvd_l0[mbPartIdx][subMbPartIdx][compIdx]);
				}
			}
		}
		...
		if (mb->sub_mb_type[mbPartIdx] != B_Direct_8x8
			...
			for (int subMbPartIdx = 0;
				...
				for (int compIdx = 0; compIdx < 2; compIdx++) {
					...
					printf(
							"mb->mvd_l1[ mbPartIdx ][ subMbPartIdx ][ compIdx ]: %d \n",
							mb->mvd_l1[mbPartIdx][subMbPartIdx][compIdx]);
				}
			}
		}
...
int NextMbAddress( int CurrMbAddr ) {return 0;}
int MbPartPredMode( int mb_type, int mbPartIdx ) {return 0;}
int NumMbPart( int mb_type ) {return 0;}
int NumSubMbPart( int sub_mb_type ){return 0;}
int SubMbPredMode( int sub_mb_type ){return 0;}
int TotalCoeff( int x ){return 0;}
int TrailingOnes( int x ){return 0;}
int Min( int a, int b ){return 0;}
int Abs( int x ){return 0;}

uint32_t bs_read_te(bs_t* b){return 0;}
void bs_write_te(bs_t* b, uint32_t v){return;}
uint32_t bs_read_me(bs_t* b){return 0;}
void bs_write_me(bs_t* b, uint32_t v){return;}
uint32_t bs_read_ae(bs_t* b) { return 0;}
uint32_t bs_read_ce(bs_t* b) { return 0;}

void read_debug_residual_block( bs_t* b, int* coeffLevel, int maxNumCoeff ){return 0;}
