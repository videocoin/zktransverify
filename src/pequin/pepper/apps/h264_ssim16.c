...
void ssim_4x4x2_core(pixel *pix1, pixel *pix2, int *sums )
{
    int z, y, x;
    for (z = 0; z < 2; z++ )
    {
...
uint32_t compute(struct In *input, struct Out *output) {
    ...
    exo_compute(exo0_inputs, lens, inputs, 0);
    pixel *pix1 = input->source_mb;
    pixel *pix2 = inputs[0].values;
